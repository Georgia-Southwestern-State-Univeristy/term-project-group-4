...
| **4** | **Display API errors in UI** | TBD (Owner C) | 2 | ✅ **Partially implemented:** toast notifications already active for save/update/load. Finish by adding delete-failure toast and surfacing server validation messages (not only generic network/status errors). |
...
| **6** | **Regression tests: validation** | TBD (Owner E) | 1 (blocker) | ✅ 4 new tests: POST/PUT whitespace-only inputs → 400; existing trip creation still passes; length limits enforced |
| **7** | **UX polish: loading indicator** | TBD (Owner F) | 3 | ✅ Show spinner/progress text while checklist generates; give user visual feedback (not just disabled button) |
...
**Total:** 8 items. **Owners:** Replace `TBD (Owner X)` with assigned teammate names in sprint kickoff. Naren owns #1 (auth).
